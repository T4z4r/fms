...
@section('content')
    <style>
        ...
        .ai-commentary pre code {
            background: none;
            padding: 0;
        }

        .ai-commentary blockquote {
    ...
    <div class="container">
        ...
        @if ($selectedCostCentre && isset($analysis))
            ...
            <div class="row mb-4">
                <div class="col-md-3">
                    <div
                        class="card bg-{{ $analysis['summary']['status'] === 'over_budget' ? 'danger' : ($analysis['summary']['status'] === 'near_limit' ? 'warning' : 'success') }} text-white">
                        <div class="card-body">
                            <h6>Status</h6>
                            ...
                            <table class="excel-table table table-sm table-hover">
                                ...
                                <tbody>
                                    @foreach ($analysis['account_analysis'] as $account)
                                        <tr
                                            class="{{ $account['utilization'] > 100 ? 'table-danger' : ($account['utilization'] > 80 ? 'table-warning' : '') }}">
                                            <td>{{ $account['account_name'] }}</td>
                                            <td>£{{ number_format($account['budget']) }}</td>
                                            <td>£{{ number_format($account['actual']) }}</td>
                                            <td>
                                                <span
                                                    class="badge bg-{{ $account['utilization'] > 100 ? 'danger' : ($account['utilization'] > 80 ? 'warning' : 'success') }}">
                    ...
                    <div class="card-body">
                        @foreach ($analysis['anomalies'] as $anomaly)
                            <div class="alert alert-{{ $anomaly['severity'] === 'high' ? 'danger' : 'warning' }}">
                                <strong>{{ DateTime::createFromFormat('!m', $anomaly['month'])->format('F') }}</strong>:
                                {{ $anomaly['message'] }}
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="card-body">
                        @foreach ($analysis['recommendations'] as $rec)
                            <div class="d-flex align-items-start mb-2">
                                <span
                                    class="badge bg-{{ $rec['priority'] === 'high' ? 'danger' : 'info' }} me-2">{{ $rec['priority'] }}</span>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <p><strong>Monthly Average:</strong>
                                    £{{ number_format($analysis['forecast']['monthly_average'], 2) }}</p>
                            </div>
                            <div class="col-md-4">
                                <p><strong>Projected Remaining:</strong>
                                    £{{ number_format($analysis['forecast']['projected_spending'], 2) }}</p>
                            </div>
                            <div class="col-md-4">
                                <p><strong>Projected Total:</strong>
                                    £{{ number_format($analysis['forecast']['projected_total'], 2) }}</p>
                            </div>
                        </div>
